...
| Markdown 編集と再読み込み | 確認済み | Markdown 表示で編集、保存できることを目視確認。外部編集取り込みも目視確認で OK。 |
...
| 表示品質 | 要再確認 | Playwright で 1440px / 390px の初期表示を確認。390px の横スクロールを修正し、再確認で `html` / `body` とも overflow なし。目視確認では 390px 相当で信号ボタン上部のドラッグ移動ができなかったため、タイトルバーのドラッグ領域を追加調整。 |
...
- ダーク / ライトテーマで文字が読める
- Markdown 表示で編集して保存できる
- Markdown に未保存の変更がある状態で別ノートへ移動すると、未保存警告が表示される
- 設定画面の「アップデート確認」でクラッシュせず結果表示に留まる
...
- 検索欄、メッセージカード、投稿欄の横幅が揃っていない: チャット画面の主要要素を同じ左右余白に統一
- 390px 相当で信号ボタン上部をドラッグしてウィンドウ移動できない: タイトルバーに明示的なドラッグ領域を追加し、狭幅時の余白を調整
